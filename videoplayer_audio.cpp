...
void VideoPlayer::sdlAudioCallback(Uint8 *stream, int len) {
    ...
    while (len > 0) {
        ...
        if (_state == Stopped) {
            _aCanFree = true;//_aCanFree:音频资源是否可以释放
            break;
        }

        // _aSwrOutIdx >= _aSwrOutSize：说明当前PCM的数据已经全部拷贝到SDL的音频缓冲区了，需要解码下一个pkt，获取新的PCM数据
        if (_aSwrOutIdx >= _aSwrOutSize) {
            // 全新PCM的大小(_aSwrOutSize:音频重采样输出PCM的大小)
            _aSwrOutSize = decodeAudio();
            _aSwrOutIdx = 0;// 索引清0
            // 没有解码出PCM数据，那就静音处理
            if (_aSwrOutSize <= 0) {
                _aSwrOutSize = 1024; // 假定PCM的大小
                memset(_aSwrOutFrame->data[0], 0, _aSwrOutSize);// 给PCM填充0（静音）
            }
        }

        // 本次需要填充到stream中的PCM数据大小（解决一个PCM太大，需要多次填充）
